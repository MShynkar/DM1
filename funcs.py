...
def saveB(B: set, entryB: tk.Entry):
    B_str = list(entryB.get().split())
    B = set(int(element) for element in B_str)


def saveC(C: set, entryC: tk.Entry):
    C_str = list(entryC.get().split())
    C = set(int(element) for element in C_str)
...
def step(window: tk.Tk, inA: set, inB: set, inC: set, inU: set):
    ...
    if window.step_counter == 1:
        ...
        temporary_result = nott(inA).union(inB).union(nott(inC)).union(inB.intersection(nott(inC)))
        Label7 = tk.Label(window, text=f"D = A̅ ∪ B ∪ C̅ ∪ (B ∩ C̅) = {temporary_result}")
        Label7.pack()
    elif window.step_counter == 4:
        temporary_result = nott(inA).union(inB).union(nott(inC)).union(inB.intersection(nott(inC))).union(inC.intersection(nott(inA)))
        Label8 = tk.Label(window, text=f"D = A̅ ∪ B ∪ C̅ ∪ (B ∩ C̅)∪ (A̅ ∩ C) = {temporary_result}")
        Label8.pack()
    elif window.step_counter == 5:
        temporary_result = nott(inA).union(inB).union(nott(inC)).union(inB.intersection(nott(inC))).union(inC.intersection(nott(inA))).union(inA.intersection(inB))
        Label9 = tk.Label(window, text=f"D = A̅ ∪ B ∪ C̅ ∪ (B ∩ C̅) ∪ (A̅ ∩ C) ∪ (A ∩ B) = {temporary_result}")
        Label9.pack()
